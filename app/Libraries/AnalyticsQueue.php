...
final class AnalyticsQueue
{
    ...
    public function enqueue(array $payload): bool
    {
        try {
            ...
            $target = $pendingDirectory . DIRECTORY_SEPARATOR
                . gmdate('YmdHis') . '-' . bin2hex(random_bytes(8)) . '.json';
    ...
    {
        return rtrim($this->directory, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . self::PENDING_DIRECTORY;
    }

    private function failedDirectory(): string
    {
        return rtrim($this->directory, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . self::FAILED_DIRECTORY;
    }
    ...
    /** @return array{version:int,queued_at:string,attempts:int,next_attempt_at:string|null,payload:array<string,mixed>}|null */
