...
$pdfUrl = esc($data['pdf_file_url'] ?? $data['pdf_file'] ?? '');
$height = esc($config['height'] ?? '600px');
$allowDownload = (bool) ($config['allow_download'] ?? true);
...
<section class="section-sm <?= esc($cssClass) ?>">
    <div class="max-w-5xl mx-auto px-4">
        ...
        <!-- Object PDF Embed Wrapper -->
        <div class="relative w-full overflow-hidden border border-slate-200/80 rounded-2xl bg-slate-100 shadow-sm" style="height: <?= $height ?>;">
            <object data="<?= $pdfUrl ?>" type="application/pdf" class="w-full h-full">
                <!-- Mobile Fallback / Non-supported browser fallback -->
                <div class="flex flex-col items-center justify-center h-full p-8 text-center bg-slate-50">
                    ...
                    <a href="<?= $pdfUrl ?>" target="_blank" rel="noopener noreferrer" class="btn btn-secondary inline-flex items-center gap-1.5 text-xs font-semibold px-4 py-2 rounded-xl border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 shadow-sm transition-all">
        ...
        <?php if ($allowDownload): ?>
            <div class="mt-4 flex justify-end">
                <a href="<?= $pdfUrl ?>" 
                   download
                   class="inline-flex items-center gap-2 text-xs font-semibold text-violet-600 hover:text-violet-700 transition-colors">
